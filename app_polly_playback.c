 ...
 *
 * By Vinicius Souza <vmsouza@vmsouza.com>
 *
...
/*! \file
 *
 * \brief Caching AWS Polly application to playback a tts
 *
...
/*** DOCUMENTATION
	<application name="PollyPlayback" language="en_US">
		<synopsis>
			Play a file.
		</synopsis>
		<syntax>
		...
		<description>
			<para>Plays back given string text to tts using AWS Polly
			The pollyplayback command answer the channel if no options are specified.
			If the file is non-existant it will try to synth with AWS Polly</para>
...
const char * strtolower(char *data);
const char * strtolower(char *data) {
...
static struct ast_config *polly_cfg = NULL;
static char *cache_dir;

static char *app = "PollyPlayback";
...
static int playback_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log(LOG_WARNING, "PollyPlayback requires an argument (string)\n");
		return -1;
	}
	...
	if (!res) {
	    ...
	    char cachedfname[33]; // 32 characters + .gsm extension + null terminator
		bytes2md5(strtolower(args.string), strlen(args.string),cachedfname);

		char fname[1024];
		strncpy(fname, cache_dir,1023);
		sprintf(fname,"%s/%s.gsm",cache_dir,(char *)cachedfname);
		//ast_log(LOG_NOTICE,"fname = %s\n",fname);
		if (access(fname, F_OK) != 0) {
			ast_log(LOG_NOTICE,"Sound not stored in cache, generating...\n");
		} else {
			ast_log(LOG_NOTICE,"Found on cache...%s\n",fname);
		}

		//char *back = args.string;
		strncpy(fname, cache_dir,1023);
		sprintf(fname,"%s/%s",cache_dir,(char *)cachedfname);
		char *back = fname;
		char *front;
...
static int reload(void)
{
	...
	reload=1;
	if ((newcfg = ast_config_load("polly.conf", config_flags)) == CONFIG_STATUS_FILEUNCHANGED) {
		reload=0;
	} else if (newcfg == CONFIG_STATUS_FILEINVALID) {
		ast_log(LOG_ERROR, "Config file polly.conf is in an invalid format.  Aborting.\n");
		reload=0;
	}

	if (reload) {
		if (polly_cfg) {
			ast_config_destroy(polly_cfg);
			ast_log(LOG_NOTICE, "Reloading polly.conf\n");
		}
		polly_cfg = newcfg;
		if (polly_cfg) {
			for (v = ast_variable_browse(polly_cfg, "general"); v ; v = v->next) {
	    			if (ast_extension_match(v->name, "cache_dir")) {
						ast_log(LOG_NOTICE, "Cache Dir: %s\n",v->value);
						cache_dir=(char *)v->value;
					break;
				}
			}
		}
...
static int load_module(void)
{
	...
	polly_cfg = ast_config_load("polly.conf", config_flags);
	if (polly_cfg && say_cfg != CONFIG_STATUS_FILEINVALID) {
		for (v = ast_variable_browse(polly_cfg, "general"); v ; v = v->next) {
    			if (ast_extension_match(v->name, "cache_dir")) {
				ast_log(LOG_NOTICE, "Cache Dir: %s\n",v->value);
				cache_dir=(char *)v->value;
...
AST_MODULE_INFO(ASTERISK_GPL_KEY, AST_MODFLAG_DEFAULT, "Sound Polly Cache Playback Application",
